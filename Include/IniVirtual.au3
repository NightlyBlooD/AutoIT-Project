...
; версия 0.5 от 2013.01.18
; http://www.autoitscript.com/forum/topic/147373-inivirtual

; Формат возвращаемых значений и ошибок максимально приближен к нативным функциям (например ошибки записи быть не может, так как данные пишутся в памяти).
...
Func _IniVirtual_Read($a_Ini_Virtual2D, $sSection, $sKey, $sDefault = '')
	...
	$i = _ArraySearch($aKey, $sKey, 1, 0, 0, 0, 1, 0)
	If @error Then Return $sDefault
	Return $aKey[$i][1]
...
Func _IniVirtual_WriteSection(ByRef $a_Ini_Virtual2D, $sSection, $vData, $iIndex = 1)
	...
	EndIf
	Local $i = _ArraySearch($a_Ini_Virtual2D, $sSection, 1, 0, 0, 0, 1, 0)
	If @error Then ; Если секция не существует, выделяем ячеку для неё
		$a_Ini_Virtual2D[0][0] += 1
		ReDim $a_Ini_Virtual2D[$a_Ini_Virtual2D[0][0] + 1][2]
		$i = $a_Ini_Virtual2D[0][0] ; индекс новой ячейки
	EndIf
	$a_Ini_Virtual2D[$i][0] = $sSection
	If $u Then
		If $iIndex = 1 Then
	...
	Else
		$a_Ini_Virtual2D[$i][1] = __IniVirtual_GetKeysVal(StringRegExpReplace($vData, '(?<!\r)\n', @CRLF))
	EndIf
	Return 1
...
Func _IniVirtual_RenameSection(ByRef $a_Ini_Virtual2D, $sSection, $sNewName, $flag = 0)
	...
	Local $i = _ArraySearch($a_Ini_Virtual2D, $sSection, 1, 0, 0, 0, 1, 0)
	If @error Then Return SetError(1, 0, 0) ; если секции для переименования не существует, то выход с @error
	Local $j = _ArraySearch($a_Ini_Virtual2D, $sNewName, 1, 0, 0, 0, 1, 0)
	If @error Then
		$a_Ini_Virtual2D[$i][0] = $sNewName ; если новый раздел не существует, то просто меняем имя раздела
	Else
		If $flag Then
			$a_Ini_Virtual2D[$j][1] = $a_Ini_Virtual2D[$i][1] ; если перезапись, то копируем старый массив в позицию нового
			; _IniVirtual_Delete($a_Ini_Virtual2D, $sSection) ; а старый удаляем
			__IniVirtual_Delete2D($a_Ini_Virtual2D, $i)
		Else
...
Func _IniVirtual_Save($a_Ini_Virtual2D)
	...
	For $i = 1 To $a_Ini_Virtual2D[0][0]
		...
		For $j = 1 To $aKey[0][0]
			$aKey[$j][1] = StringRegExpReplace($aKey[$j][1], '^([''"]).*\1\z', '"\0"')
...
Func _IniVirtual_IsDuplicateKeys($a_Ini_Virtual2D, $sSection)
	...
	If @error Then Return SetError(1, 0, 0)
	Local $aKey = $a_Ini_Virtual2D[$i][1]
	$sRes = _IniVirtual_IsDuplicateSection($aKey)
...
Func _IniVirtual_IsDuplicateSection($aArr2D)
	...
	For $i = 1 To $aArr2D[0][0]
		If IsDeclared($aArr2D[$i][0] & $s) Then
			If $aArr2D[$i][0] == '' Then Return SetError(1, 0, True) ; присутсвтует пустое имя секции
